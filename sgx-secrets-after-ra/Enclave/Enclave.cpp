...
// Create new connection to `server:port`
// Receive `secret` and print it
sgx_status_t ecall_receive_secret(const char *server, const char *port)
{
    ocall_print_string("Entered ecall_receive_secret\n");

    // Print params for debugging
    // ocall_print_string("server: ");
    // ocall_print_string(server);
    // ocall_print_string("\n");

    // ocall_print_string("port: ");
    // ocall_print_string(port);
    // ocall_print_string("\n");


    // Now create connection to `server` using an OCALL
    // and actually receive secret.
    ocall_print_string("Calling ocall_receive_secret_msgio\n");
    sgx_status_t ret = ocall_receive_secret_msgio(server, port);
    ocall_print_sgx_status("ocall_receive_secret_msgio returned:", ret);
