...
int do_attestation (sgx_enclave_id_t eid, config_t *config)
{
    ...
    // We can't pass MSGIO object to the enclave directly
    // We have to pass `server:port` and recreate connection with `server` from
    // withing the enclave call.
...
void ocall_print_sgx_status(const char *msg, sgx_status_t s)
{
    printf("%s %i", msg, s);
}


void ocall_receive_secret_msgio(const char *server, const char *port)
{
    printf("Entered ocall_receive_secret_msgio\n");
    ...
    msgio = new MsgIO(server, port);

    printf("Reading from msgio\n");
    rv = msgio->read((void **)&secret, &secret_sz);
    if (rv == 0)
    {
        fprintf(stderr, "Protocol error reading secret\n");
        delete msgio;
        exit(1);
    }
    else if (rv == -1)
    {
        fprintf(stderr, "System error occurred while reading secret\n");
        delete msgio;
        exit(1);
    }
    fprintf(stderr, "Received secret from server: %s (%lu)\n", secret, secret_sz / 2);
}
