...
int main(int argc, char *argv[])
{
    ...
    fprintf(stderr, "Starting server for uRTS\n");
	if ( flag_stdio ) {
		msgio= new MsgIO();
	} else {
		try {
	...
	while ( msgio->server_loop() ) {
        ...
        fprintf(stderr, "Received msg4 with status: %d\n", msg4.status);

        // At this point, we received walid msg4, i.e. enclave at `client` is secure
        // TODO: Notice that currently we trust msg4 even if status is `NotTrusted_ItsComplicated`
...
/* Windows implementation is not available yet */
		...
		if ( msg4->status == Trusted || msg4->status == NotTrusted_ItsComplicated) { // TODO: Run only when status is Trusted
